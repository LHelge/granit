...
- **The `.granit/` directory** is excluded as well. It holds the cave's `config.yml`, its `templates/` and `presentations/` folders, and the agent's files under `agent/` (the [[system-prompt]], [[skills-and-tasks|skills, and tasks]]) rather than notes — see [[configuration]] for what lives there.

Templates are deliberately kept outside the note tree in `.granit/templates/` and use their own separate, flat slug namespace, so a template name never collides with a note name. See [[templates]] for details. [[presentations|Presentation templates]] follow the same rule in `.granit/presentations/`.
