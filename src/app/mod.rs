...
use components::release_notes::ReleaseNotesModal;
...
pub fn App() -> impl IntoView {
    ...
    // Version installed in the background this session, awaiting a restart.
    let (installed_update, set_installed_update) = signal(None::<String>);
    ...
    // Show release notes on the first launch after an automatic update.
    leptos::task::spawn_local(async move {
        if let Ok(Some(notes)) = ipc::fetch_pending_release_notes().await {
            set_release_notes.set(Some(notes));
        }
    });
    ...
    view! {
        ...
        >
            ...
            {move || {
                release_notes
                    .get()
                    .map(|notes| view! { <ReleaseNotesModal notes set_notes=set_release_notes /> })
            }}

            // Toast notifications (bottom-right)
            <div class="toast toast-end toast-bottom z-50">
                <Show when=move || installed_update.get().is_some()>
                    <div role="alert" class="alert alert-info alert-soft alert-sm shadow-lg max-w-sm">
                        <span class="flex-1 text-xs leading-relaxed">
                            {move || {
                                let version = installed_update.get().unwrap_or_default();
                                format!("Granit v{version} was installed — restart to apply")
                            }}
                        </span>
                        <button
                            ...
                            on:click=move |_| {
                                leptos::task::spawn_local(async move {
                                    if let Err(e) = ipc::restart_app().await {
                                        ctx.push_error("updater", format!("Failed to restart: {e}"));
                                    }
                                });
                            }
                        >
                        ...
                        <button
                            class="btn btn-ghost btn-xs btn-square"
                            on:click=move |_| set_installed_update.set(None)
                        >
                            "✕"
                        </button>
                    </div>
