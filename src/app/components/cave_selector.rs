...
pub fn CaveSelector(set_settings_open: WriteSignal<bool>) -> impl IntoView {
    ...
    view! {
        <div class="border-t border-edge-subtle px-2 py-2">
            <div class="flex items-center gap-1">
                // Cave selector dropdown
                <div class="relative flex-1">
                    <button
                        class="w-full flex items-center justify-between px-2 py-1.5 text-sm bg-card border border-edge rounded hover:border-edge-hover transition-colors text-fg-secondary text-left truncate"
                        on:click=move |_| set_dropdown_open.update(|v| *v = !*v)
                    >
                        <span class="truncate">{cave_label}</span>
                        <span class="inline-flex w-3.5 h-3.5 ml-1 shrink-0 text-fg-muted">
                    ...
                    <Show when=move || dropdown_open.get()>
                        <div class="absolute bottom-full left-0 right-0 mb-1 bg-card border border-edge rounded shadow-lg z-50 max-h-60 overflow-y-auto">
                            // Recent caves
                            {move || {
                                let cfg = ctx.config.get();
                                cfg.recent_caves.iter().map(|path| {
                                    ...
                                    view! {
                                        <button
                                            class="w-full text-left px-3 py-1.5 text-sm text-fg-secondary hover:bg-item-hover transition-colors truncate"
                                            title=full_path
                            ...
                            <button
                                class="w-full text-left px-3 py-1.5 text-sm text-fg-secondary hover:bg-item-hover transition-colors"
                ...
                // Settings gear icon
                <button
                    class="p-1.5 rounded hover:bg-item-hover text-fg-muted hover:text-fg transition-colors"
                    title="Settings"
