            ...
            {move || {
                ...
                if is_renaming {
                    ...
                    view! {
                        <button
                            class=move || {
                                let base = "w-full text-left py-0.5 text-base text-base-content/50 hover:text-base-content transition-colors flex items-center gap-1";
                                if drag_over.get() { format!("{base} bg-base-content/8") } else { base.to_string() }
                            }
