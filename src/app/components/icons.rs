...
#[component]
pub fn ChevronIcon(#[prop(into)] open: Signal<bool>) -> impl IntoView {
    view! {
        <svg
            xmlns="http://www.w3.org/2000/svg"
            class=move || {
                if open.get() {
                    "w-3 h-3 shrink-0 transition-transform rotate-90"
                } else {
...
///
/// Accepts an optional `class` override (default `"w-3.5 h-3.5"`) and an
/// optional reactive `open` signal that adds `rotate-180` when true.
#[component]
pub fn ChevronDownIcon(
    #[prop(default = "w-3.5 h-3.5")] class: &'static str,
    ...
    view! {
        <svg
            xmlns="http://www.w3.org/2000/svg"
            class=class
            class=("rotate-180", move || open.is_some_and(|s| s.get()))
...
/// X / close icon (✕).
#[component]
pub fn XCloseIcon() -> impl IntoView {
    view! {
        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
/// Eye-slash icon — password hide (closed).
#[component]
pub fn EyeSlashIcon() -> impl IntoView {
    view! {
        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
