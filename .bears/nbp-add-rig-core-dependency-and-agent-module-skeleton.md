...
updated: 2026-03-28T22:51:17.592991759Z
tags:
- backend
parent: ann
---
...
Add `rig-core` and `tokio` to the backend, create the `src-tauri/src/agent/` module with an Ollama client builder. Update `AgentConfig` defaults to `ollama`/`qwen3.5:9b`.
...
- [ ] `AgentConfig` updated: add `base_url: Option<String>` field, default provider changed to `"ollama"`, default model to `"qwen3.5:9b"`
